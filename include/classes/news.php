	...
	public function dataview($query, $sure, $web_admin, $news_lvl, $site_url, $read_more)
	{
		...
		if($rowCount>0)
		{
			while($row=$stmt->fetch(PDO::FETCH_ASSOC))
			{
				?>
					<div class="news">
						<a href="<?php print $site_url; ?>read/<?php print $row['id']; ?>"><?php print $row['title']; ?></a>
						<?php if($web_admin>=$news_lvl) { ?><a href="<?php print $site_url; ?>?delete=<?php print $row['id']; ?>" onclick="return confirm('<?php print $sure; ?>');"><i style="color:red;" class="fa fa-trash-o fa-2" aria-hidden="true"></i></a><?php } ?>
						<span class="date"><?php print date("d.m.Y", strtotime($row['time'])); ?></span>
					</div>
                <?php
			}
		}
